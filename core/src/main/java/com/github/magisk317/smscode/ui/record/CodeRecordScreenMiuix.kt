 ...
 * the pager-coordinated record list. The measured bar height is fed back as
 * the list's top padding so content never hides under the overlay header,
 * and the pager header offset keeps translating the bar while swiping tabs.
 * When the theme backdrop is active the bar frosts over the recorded list
 * (same glass model as OverviewScreenMiuix / MiPush EventListMiuix).
